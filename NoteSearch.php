...
<h3><a href="./"><?php echo $Title;?></a> <svg width="16" height="16" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"><polygon fill="#fff" points="20,15 80,50 20,85" fill="black" /></svg> Note Search</h3>
</div>
</div>
...
$result = $db->prepare("SELECT Notes_id, Notes_name, Notes_content FROM Notes WHERE Notes_name LIKE :search OR Notes_content LIKE :search ORDER BY Notes_TimeStamp_Modified DESC LIMIT '$start', '$limit'");
$result->execute([':search' => '%' . $searchTerm . '%']);
$i = 1;
...
if(!isset($page))
$page=1;
echo pagination($total_pages,$limit,$page,$_SERVER["PHP_SELF"]."?search=$searchTerm&page_id=",$adjacents);
if(empty($rows))
{
...
<div id="Warningalignright">No results were found for your search query!</div>
</div>';
}
?>
</div>
